...
import com.jeecms.common.util.PropertieUtil;
...
public class Config extends BaseConfig {
	...
	public static class ConfigEmailSender implements EmailSender {
		...
		public static ConfigEmailSender createByProperties(HttpServletRequest request) {
			//从properties文件中取得配置值
			PropertieUtil pu=new PropertieUtil();
			String email_host=pu.readData("config/mail.properties", "email_host", request);
			String email_port=pu.readData("config/mail.properties", "email_port", request);
			String email_encoding=pu.readData("config/mail.properties", "email_encoding", request);
			String email_username=pu.readData("config/mail.properties", "email_username", request);
			String email_password=pu.readData("config/mail.properties", "email_password", request);
			String email_personal=pu.readData("config/mail.properties", "email_personal", request);
			String email_title=pu.readData("config/mail.properties", "email_title", request);
			String email_text=pu.readData("config/mail.properties", "email_text", request);
			String return_url=pu.readData("config/mail.properties", "return_url", request);
			
			Map<String, String> map=new HashMap<String, String>();
			
			map.put("email_host", email_host);
			map.put("email_port", email_port);
			map.put("email_encoding", email_encoding);
			map.put("email_username", email_username);
			map.put("email_password", email_password);
			map.put("email_personal", email_personal);
			map.put("email_title", email_title);
			map.put("email_text", email_text);
		...
		public String getTitle() {
			String title = getAttr().get("email_title");
			return StringUtils.isNotBlank(title) ? title : null;
		}

		@Override
		public String getContent() {
			String email_text = getAttr().get("email_text");
			return StringUtils.isNotBlank(email_text) ? email_text : null;
		}

		@Override
		public void setContent(String email_text) {
			getAttr().put("email_text", email_text);
		}
		
		@Override
		public String getReturnUrl() {
			return getAttr().get("return_url");
		}
	}

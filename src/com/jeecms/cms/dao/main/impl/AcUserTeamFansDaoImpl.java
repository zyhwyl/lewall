...
@Repository
public class AcUserTeamFansDaoImpl extends HibernateBaseDao<AcUserTeamFans, Integer>
		implements AcUserTeamFansDao {
	...
	public AcUserTeamFans deleteById(Integer id) {
		AcUserTeamFans entity=super.get(id);
		if (entity != null) {
	...
	@Override
	protected Class<AcUserTeamFans> getEntityClass() {
		return AcUserTeamFans.class;
	}

	@Override
	public Pagination getPaginationByProperities(String[] keys,
			Object[] values, int pageNo, int pageSize) {
		Finder f = Finder.create("from AcUserTeamFans bean");
		f.append(" where ");
	...
	public AcUserTeamFans findById(Integer id) {
		return (AcUserTeamFans)getSession().get(AcUserTeamFans.class, id);
	}
	...
	public List getListByProperities(String[] keys,
		...
		String sql = "from AcUserTeamFans ";
		sql+=" where ";
		if (keys!=null&&keys.length>0) {
			int i=0;
			for(String key:keys){

...
@Repository
public class AcUserTeamTagDaoImpl extends HibernateBaseDao<AcUserTeamTag, Integer>
		implements AcUserTeamTagDao {
	...
	public AcUserTeamTag deleteById(Integer id) {
		AcUserTeamTag entity=super.get(id);
		if (entity != null) {
	...
	@Override
	protected Class<AcUserTeamTag> getEntityClass() {
		return AcUserTeamTag.class;
	}

	@Override
	public Pagination getPaginationByProperities(String[] keys,
			Object[] values, int pageNo, int pageSize) {
		Finder f = Finder.create("from AcUserTeamTag bean");
		f.append(" where ");
	...
	public AcUserTeamTag findById(Integer id) {
		return (AcUserTeamTag)getSession().get(AcUserTeamTag.class, id);
	}
	...
	public AcUserTeamTag findByName(String tagName) {
		return this.findUniqueByProperty("TagName", tagName);
	}
}
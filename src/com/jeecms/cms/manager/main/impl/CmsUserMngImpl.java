...
public class CmsUserMngImpl implements CmsUserMng {
	...
	public static String getValicode() {
		...
		for (int i = 0; i < 24; i++) {			
			...
			valicode=valicode+temp;
		}
		SimpleDateFormat format=new SimpleDateFormat("MMddHHmmssMM");
		Date date=new Date();
		String now=format.format(date); 
		return now+valicode.toUpperCase();
	}
	
	public CmsUser registerMember(String username, String email,
			String password, String ip, Integer groupId, CmsUserExt userExt,
			Boolean activation, EmailSender sender)throws UnsupportedEncodingException, MessagingException{
		UnifiedUser unifiedUser = unifiedUserMng.save(username, email,
				password, ip, activation, sender);
		CmsUser user = new CmsUser();
		user.forMember(unifiedUser);
		...
		url=StringUtils.replace(url, "${userid}",user.getId().toString());
		url=StringUtils.replace(url, "${email}",user.getEmail());
		//生成valicode
		String valicode=unifiedUser.getActivationCode();
	...
	@Override
	public List<CmsUser> findUserBySchool( String[] keys,
			Object[] values, Integer count) {
		return dao.findUserBySchool(keys, values, count);
	}
	...
	@Override
	public CmsUser registerQQMember(String username, String email,
			String openid, String ip, Integer groupId, CmsUserExt userExt,
			Boolean activation, EmailSender sender)
			throws UnsupportedEncodingException, MessagingException {
		UnifiedUser unifiedUser = unifiedUserMng.qqsave(username, email,
				openid, ip, activation, sender);
		CmsUser user = new CmsUser();
		user.forMember(unifiedUser);
